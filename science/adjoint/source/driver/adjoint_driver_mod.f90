...
!-----------------------------------------------------------------------------
!>@brief Drives the execution of the adjoint model.
module adjoint_driver_mod
  ...
  use linear_model_data_mod,      only : linear_create_ls, &
                                         linear_init_ls
  use adjoint_model_data_mod,     only : adjoint_init_pert
...
contains
  ...
  !>
  subroutine initialise( program_name, modeldb, atl_si_timestep )
    ...
    call initialise_adjoint_model( modeldb, atl_si_timestep )
  ...
  subroutine step( modeldb, atl_si_timestep )
  ...
  !> @param [in]     program_name An identifier given to the model being run
  !> @param [in,out] modeldb   The structure that holds model state
  subroutine finalise( program_name, modeldb, atl_si_timestep )
